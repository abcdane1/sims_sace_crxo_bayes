...
#source("/home/User/crxodatasimlogitsepv2_harm_gh.R")
...
sacecrxo_logitwrapper<-function(replic,total_iterations,burn_in_iterations,
                                I,J,csl,csu,mux,zp,wp,hp,
                                y111,y110,y101,y010,
                                sigma2y11,sigma2y10,sigma2y01,
                                sdscz,sdscpz,sdscw,sdscpw,sdsch,sdscph,
                                sdyc11,sdycp11,sdyc10,sdycp10,sdyc01,sdycp01,sec,
                                trtp,logscale,rr,initv){
  
  dfs<-crxodfsimlogit_v2_harm(I=I,J=J,csl=csl,csu=csu,mux=mux,zp=zp,wp=wp,hp=hp,
                              y111=y111,y110=y110,y101=y101,y010=y010,
                              sigma2y11=sigma2y11,sigma2y10=sigma2y10,sigma2y01=sigma2y01,
                              sdscz=sdscz,sdscpz=sdscpz,sdscw=sdscw,sdscpw=sdscpw,sdsch=sdsch,sdscph=sdscph, 
                              sdyc11=sdyc11,sdycp11=sdycp11,sdyc10=sdyc10,sdycp10=sdycp10,sdyc01=sdyc01,sdycp01=sdycp01,
...
sigma2y01<-rep(2.364689,12) #var on log-scale .75

#BPC.01,WPC.02 and ICCs=.02 (add harmed patients 01 to all)
...
var_scen1_out01<-icctovar_fun(.01,.02,inv_lognormal(sigma2y01[1],0))

var_log_scale_scen1_out11<-lognormal_var(var_scen1_out11,0)
var_log_scale_scen1_out10<-lognormal_var(var_scen1_out10,0)
var_log_scale_scen1_out01<-lognormal_var(var_scen1_out01,0)
...
var_scen2_out01<-icctovar_fun(.03,.035,inv_lognormal(sigma2y01[1],0))

var_log_scale_scen2_out11<-lognormal_var(var_scen2_out11,0)
var_log_scale_scen2_out10<-lognormal_var(var_scen2_out10,0)
var_log_scale_scen2_out01<-lognormal_var(var_scen2_out01,0)
...
var_scen3_out01<-icctovar_fun(.05,.1,inv_lognormal(sigma2y01[1],0))

var_log_scale_scen3_out11<-lognormal_var(var_scen3_out11,0)
var_log_scale_scen3_out10<-lognormal_var(var_scen3_out10,0)
var_log_scale_scen3_out01<-lognormal_var(var_scen3_out01,0)
...
sdyc01<-rep(sqrt(c(var_log_scale_scen1_out01[1],var_log_scale_scen2_out01[1],var_log_scale_scen3_out01[1])),4)
sdycp01<-rep(sqrt(c(var_log_scale_scen1_out01[2],var_log_scale_scen2_out01[2],var_log_scale_scen3_out01[2])),4)
initv_vect<-rep(c(1,2,3),4) #initial variance 

zp<-c(.2,.2,-.4,.1,.05) #z logit coefficients 35,25,40
wp<-c(-.1,-.4,-.3,-.1,.025) #w logit coefficients
hp<-c(-2.7,-.5,-.6,-.4,.05) #2.5% #adds harmed patients 
# hp<-c(-2,-.5,-.6,-.4,.05) #5%
# hp<-c(-1.05,-.5,-.6,-.4,.05) #10%

rr<-T
logscale<-T

#app tables, harmed patients, survival monotonicity violation  

#scenarios, scale is the amount y01(0) is shifted away from y11(0) (additively on log_scale), 3 and 4 coprime so lists everything
scenariomat<-cbind(csize=I,csl=csl,csu=csu,bpc=rep(c(.01,.03,.05),4),wpc=rep(c(.02,.035,.1),4),scale=rep(c(.1,.3,.5,.7),3))
...
for(i in (1:nrow(scenariomat))){
  ...
  l<-parallel::mcmapply(sacecrxo_logitwrapper,replic=1:nsim,
                        MoreArgs=list(total_iterations=total_iterations,burn_in_iterations=burn_in_iterations,
                                      ...
                                      mux=mux,zp=zp,wp=wp,hp=hp,y111=y111,y110=y110,y101=y101,y010=c(y110[1:4]+scenariomat[i,"scale"],y110[5]),
                                      sigma2y11=sigma2y11[i],sigma2y10=sigma2y10[i],sigma2y01=sigma2y01[i],
                                      sdscz=sdscz[i],sdscpz=sdscpz[i],sdscw=sdscw[i],sdscpw=sdscpw[i],sdsch=sdsch[i],sdscph=sdscph[i],
                                      sdyc11=sdyc11[i],sdycp11=sdycp11[i],sdyc10=sdyc10[i],sdycp10=sdycp10[i],sdyc01=sdyc01[i],sdycp01=sdycp01[i],
                                      sec=T,trtp=.5,logscale=T,rr=rr,initv=initv_vect[i]),
                        mc.cores=60)
  
  truedf<-crxodfsimlogit_v2_harm(I=5000,J=2,csl=scenariomat[i,"csl"],csu=scenariomat[i,"csu"],
                                 mux=mux,zp=zp,wp=wp,hp=hp,y111=y111,y110=y110,y101=y101,y010=c(y110[1:4]+scenariomat[i,"scale"],y110[5]),
                                 sigma2y11=sigma2y11[i],sigma2y10=sigma2y10[i],sigma2y01=sigma2y01[i],
                                 sdscz=sdscz[i],sdscpz=sdscpz[i],sdscw=sdscw[i],sdscpw=sdscpw[i],sdsch=sdsch[i],sdscph=sdscph[i],
                                 sdyc11=sdyc11[i],sdycp11=sdycp11[i],sdyc10=sdyc10[i],sdycp10=sdycp10[i],sdyc01=sdyc01[i],sdycp01=sdycp01[i],
  ...
  gtrue<-truedf$g
  gperctrue<-stratacomp_fun_harm(gtrue)[2:4] #p00,p10,p11 
  ...
  if(i %% 1 == 0){
    filename<-paste("crxologlogit_sim_table_harm_2p5",i,".csv",sep="")
