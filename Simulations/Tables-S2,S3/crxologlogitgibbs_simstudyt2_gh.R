...
sacecrxo_logitwrapper<-function(replic,total_iterations,burn_in_iterations,
  ...
  output1<-check_fail(func=sace_truecrxologitv3_noop_full,data=dfsim,trt="a",surv="s",out="y",ind=c("x1","x2","x3"),clustid="id",clustp="j",cpsize="nij",
                      ...
                      credbound=c(.025,.975),logscale=logscale,rr=rr,cs=T,cps=T,cy=T,cpy=T,initv=initv)
  ...
  #just repeat values to allow for same table format 
  output3<-output1
  output4<-output2
  
  #commented out as not needed in table 
...
sigma2y11<-rep(4.670774,3) #var log-scale 1
sigma2y10<-rep(8.692151,3) #var log-scale 1.25

#BPC.01,WPC.02 and ICCs=.02
var_scen1_ps<-icctovar_fun(.01,.02,pi^2/3)
...
var_scen2_ps<-icctovar_fun(.03,.035,pi^2/3)
...
var_scen3_ps<-icctovar_fun(.05,.1,pi^2/3)
...
#table app, but not included in results
# zp<-c(.7,.2,-.35,.1,.05) #z logit coefficients 25,25,50
...
for(i in c(1,2,3)){
  ...
  for(j in 1:4){
    ...
    #component means 
    #ppcomp<-apply(l[8:9+nmetrics*(j-1),sucsaceestim],1,mean)
    #gperctruemean<-apply(l[8:10+nmetrics*(j-1),],1,mean)
  ...
  if(i %% 1 == 0){
    filename<-paste("crxologlogit_sim_apptable2new_352540",i,".csv",sep="")
