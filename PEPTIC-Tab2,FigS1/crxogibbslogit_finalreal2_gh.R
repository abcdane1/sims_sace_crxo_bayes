sace_truecrxologitv3_noop_full_real<-function(data,trt="a",surv="s",out="y",ind="x",clustid="id",clustp="j",cpsize="nij",sec=F,
  ...
  #cluster periods, two periods for NOW 
  IM<-I*J
  ...
  clusta1<-sort(unique(clustid[dfsim$a==1]))
  clustpa10<-sort(unique(clustpind))
  ...
  single_chain_fun<-function(ch){
    ...
    #randomized
    tau2cz_up0<-(sdscz^2)[initv] 
    tau2cz_up<-runif(1,.5*tau2cz_up0,1.5*tau2cz_up0)
    tau2cw_up0<-(sdscw^2)[initv] 
    tau2cw_up<-runif(1,.5*tau2cw_up0,1.5*tau2cw_up0)
    
    #initial variance of cluster period rand int for survival
    #randomized
    tau2cpz_up0<-(sdscpz^2)[initv] 
    tau2cpz_up<-runif(1,.5*tau2cpz_up0,1.5*tau2cpz_up0)
    tau2cpw_up0<-(sdscpw^2)[initv] 
    tau2cpw_up<-runif(1,.5*tau2cpw_up0,1.5*tau2cpw_up0)
    
    ##non-mortal outcome initial values
    
    #initial variance of non-mortal outcome models
    #randomized
    sigma2_up11_0<-ifelse(logscale==F,sigma2y11[initv],inv_lognormal(sigma2y11[initv],0)) #updated from 1 
    sigma2_up11<-runif(1,.5*sigma2_up11_0,1.5*sigma2_up11_0)
    
    sigma2_up10_0<-ifelse(logscale==F,sigma2y10[initv],inv_lognormal(sigma2y10[initv],0))
    sigma2_up10<-runif(1,.5*sigma2_up10_0,1.5*sigma2_up10_0)
    ...
    sigma2c11_up0<-ifelse(logscale==F,sdyc11^2,inv_lognormal((sdyc11^2)[initv],0))
    sigma2c11_up<-runif(1,.5*sigma2c11_up0,1.5*sigma2c11_up0)
    
    #init value cluster period rand int variance non-mortal
    sigma2cp11_up0<-ifelse(logscale==F,sdycp11^2,inv_lognormal((sdycp11^2)[initv],0))
    sigma2cp11_up<-runif(1,.5*sigma2cp11_up0,1.5*sigma2cp11_up0)
    
    #init value cluster rand int variance non-mortal
    sigma2c10_up0<-ifelse(logscale==F,sdyc10^2,inv_lognormal((sdyc10^2)[initv],0))
    sigma2c10_up<-runif(1,.5*sigma2c10_up0,1.5*sigma2c10_up0)
    
    #init value cluster period rand int variance non-mortal
    sigma2cp10_up0<-ifelse(logscale==F,sdycp10^2,inv_lognormal((sdycp10^2)[initv],0))
    sigma2cp10_up<-runif(1,.5*sigma2cp10_up0,1.5*sigma2cp10_up0)
    ...
    for (iter in 1:total_iterations){
      ...
      #since missing one cluster 
      Py10sub<-Py10[,clusta1]
      ...
      if(cpy==T){
        ...
        #subset to indexed 
        #missing one cluster period 
        Ly11sub<-Ly11[,clustpa10]
      ...
      if(cy==T){
        ...
        #xi of 10
        sigpart_xi10<-solve(t(Py10sub)%*%Py10sub/sigma2_up10+1/sigma2c10_up*diag(ncol(Py10sub)))
        mu_xi10<-(1/sigma2_up10)*sigpart_xi10%*%(t(Py10sub)%*%(Y10-D10y%*%theta10_up-gamma10_vect_up))
        sigma2_xi10<-sigpart_xi10
        
        xi10_up_sub<-MASS::mvrnorm(n=1,mu=mu_xi10,Sigma=sigma2_xi10)
        xi10_up[clusta1]<-xi10_up_sub
        
        #variance of xi of 10
        shapeterm_xi10v<-.001+length(clusta1)/2
        scaleterm_xi10v<-.001+t(xi10_up_sub)%*%xi10_up_sub/2
      ...
      if(cpy==T){
        ...
        sigpart_gamma11<-solve(t(Ly11sub)%*%Ly11sub/sigma2_up11+1/sigma2cp11_up*diag(ncol(Ly11sub)))
        mu_gamma11<-(1/sigma2_up11)*sigpart_gamma11%*%(t(Ly11sub)%*%(Y11-D11int%*%theta11_up-xi11_vect_up))
        sigma2_gamma11<-sigpart_gamma11
        gamma11_up_sub<-MASS::mvrnorm(n=1,mu=mu_gamma11,Sigma=sigma2_gamma11)
        gamma11_up[clustpa10]<-gamma11_up_sub
        
        #variance gamma of 11
        shapeterm_gamma11v<-.001+length(clustpa10)/2
        scaleterm_gamma11v<-.001+t(gamma11_up_sub)%*%gamma11_up_sub/2
        ...
        shapeterm_gamma10v<-.001+length(clustpa1)/2 #count clusters assigned to active treatment
        #shapeterm_gamma10v<-.001+IM/4 #in general this is equivalent to cluster-periods/2
      ...
      #p1<-ifelse(p1>.5,p1,1-p1), prevent label switching? 
